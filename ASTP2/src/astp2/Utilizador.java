 ...
 * @author Manuel
 */
public class Utilizador implements Observer,Serializable {
    ...
    public void update(HashMap<String, Double> vals) {
        ...
        for(AtivosComprados a : ativos.values()){
            if(vals.containsKey(a.getNome())){
                a.setOldValue(ativos.get(a.getNome()).getActualValue());
                ativos.get(a.getNome()).setActualValue(vals.get(a.getNome()));
                ativos.get(a.getNome()).setDif();
                updateDinheiroAtivos();
                setBalanco();
            }
        }
